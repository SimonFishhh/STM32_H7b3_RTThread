/***
   ...
   *  @description
	*
	*	实验平台：反客STM32H7B0VBT6核心板 （型号：FK7B0M1-VBT6）+ 2.00寸液晶模块（型号：SPI200M1-240*320）
...
*/
      ...
      __HAL_RCC_GPIOE_CLK_ENABLE();   // 使能 SPI GPIO 时钟
...
// 初始化 SCK、MOSI、NSS
      GPIO_InitStruct.Pin 		   = GPIO_PIN_11|GPIO_PIN_12|GPIO_PIN_14;    // SCK、MOSI、片选引脚
      GPIO_InitStruct.Mode 		= GPIO_MODE_AF_PP;            				// 复用推挽输出
      GPIO_InitStruct.Pull 		= GPIO_NOPULL;                				// 无上下拉
      GPIO_InitStruct.Speed 		= GPIO_SPEED_FREQ_VERY_HIGH;  				// 最高速度等级
      GPIO_InitStruct.Alternate  = GPIO_AF5_SPI4;              				// 复用到SPI4，复用线5
...
// SPI的内核时钟为140M，然后再经过2分频得到 70M 的SCK驱动时钟
